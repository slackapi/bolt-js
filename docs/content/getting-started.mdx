...
This quick start guide aims to help you get a Slack app using Bolt for JavaScript up and running as soon as possible!
...
You'll then land on your app's **Basic Information** page which is an overview of your app and contains important credentials.
...
3. Copy the bot token `xoxb` from the **OAuth & Permissions page** and then store it in a new environment variable:
...
With the app running, add your app's bot to a channel or send it a direct message saying "hello" and wait for a response. Clicking the attached button will post another message to channel.

## Updating the app

At this point, you're running the getting started Bolt for JavaScript [app](https://github.com/slackapi/bolt-js-getting-started-app)!
...
With intentions of the app's purpose known, return back to app settings and make changes that seem right.
...
* Explore the different events your bot can listen to with the [`event`](/concepts/event-listening) method. All of the [events](https://docs.slack.dev/reference/events) are listed on the documentation site.
* Bolt allows you to call [Web API](/concepts/web-api) methods with the client attached to your app. There are [over 200 methods](https://docs.slack.dev/reference/methods) on our API site.
* Learn more about the different [token](https://docs.slack.dev/authentication/tokens) types and [authentication](/concepts/authenticating-oauth) setups. Your app might need different tokens depending on the actions you want to perform or for installations to multiple workspaces.
* Receive events using HTTP for various deployment methods, such as to the [Heroku](/deployments/heroku) or [AWS Lambda](/deployments/aws-lambda) providers.
