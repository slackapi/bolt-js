...
| `bodyLimit` | Maximum size of an incoming request body that the receiver buffers before rejecting the request with an HTTP `413` response, guarding against large-body denial-of-service attempts. Accepts a `number` of bytes or a `bytes`-style `string` such as `'4mb'`. Defaults to `4194304` (4 MB); pass `Infinity` to disable it (not recommended in production). |
