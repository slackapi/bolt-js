...
| `bodyLimit` | Maximum size of an incoming request body -- a `number` of bytes, or a `bytes`-style `string` such as `'4mb'` -- that the receiver buffers before rejecting the request with an HTTP `413` response. This bounds per-request memory and guards against large-body denial-of-service attempts against a publicly exposed receiver. The limit is enforced on the bytes actually received (not the `Content-Length` header) and applies even when `signatureVerification` is `false`. Defaults to `4194304` (4 MB). Pass `Infinity` to disable the limit (not recommended in production). |
