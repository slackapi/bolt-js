...
exports.getRunmeLink = exports.RunmeLink = void 0;
const react_1 = __importDefault(require("react"));
const BrowserOnly_1 = __importDefault(require("@docusaurus/BrowserOnly"));
const client_1 = require("docusaurus-theme-github-codeblock/client");
...
function detectMobile(userAgent) {
    return REGEX_MOBILE_DEVICE.test(userAgent) || REGEX_VENDOR.test(userAgent.substr(0, 4));
}
function RunmeLink(props) {
    return react_1.default.createElement(BrowserOnly_1.default, null, () => react_1.default.createElement(RunmeLinkComponent, { ...props }));
}
exports.RunmeLink = RunmeLink;
function RunmeLinkComponent({ reference, metastring }) {
    const userAgent = (navigator.userAgent ||
        navigator.vendor ||
        // @ts-expect-error
        window.opera);
    const codeblockConfig = (0, client_1.useCodeblockThemeConfig)();
    const showRunmeButton = !detectMobile(userAgent) && codeblockConfig.showRunmeLink && metastring;
    if (!showRunmeButton) {
        return null;
    }
    return (react_1.default.createElement("a", { href: `vscode://stateful.runme?${getRunmeLink(reference, metastring)}`, className: 'runmeLink', target: "_blank", style: constants_1.buttonStyles }, codeblockConfig.runmeLinkLabel));
}
function getRunmeLink(snippetUrl, metastring) {
    ...
    if (snippetUrl.endsWith('.md')) {
        params.set('fileToOpen', (0, utils_1.parseReference)(snippetUrl).url);
    ...
    const { org, repo, title } = (0, utils_1.parseReference)(snippetUrl);
...
exports.getRunmeLink = getRunmeLink;
//# sourceMappingURL=RunmeLink.js.map