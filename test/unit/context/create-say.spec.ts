...
describe('createSay', () => {
  it('should call chat.postMessage with text when given a string', async () => {
    ...
    const say = createSay(client, 'xoxb-token', 'C1234');
    await say('hello');
    ...
    assert.equal(args.token, 'xoxb-token');
  });
  ...
  it('should pass undefined token when token is undefined', async () => {
    const client = new WebClient('token');
    const postMessageStub = sinon
      .stub(client.chat, 'postMessage')
      .resolves({} as Awaited<ReturnType<typeof client.chat.postMessage>>);

    const say = createSay(client, undefined, 'C1234');
    await say('hello');
